...
import { Sparkles, Loader2, RefreshCw, Calendar, Filter } from 'lucide-react';
...
const GeminiInsights: React.FC<GeminiInsightsProps> = ({ activeProfileId }) => {
  ...
  const generateInsights = async () => {
    ...
    setError(null);
    try {
      // Corrected collection names according to user spec
      ...
      if (!apiKey) {
        throw new Error("Gemini API Key tidak terdeteksi. Silakan hubungi administrator.");
      }

      const ai = new GoogleGenAI({ apiKey });
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `Analisis data bisnis (${data.filterContext}) periode (${data.periode}):
          - Unit Terjual: ${data.totalUnitTerjual}
          - Total Omset: Rp ${data.totalOmset.toLocaleString('id-ID')}
          - Postingan Konten: ${data.jumlahPostingan}
          
          Tolong berikan laporan strategis untuk UMKM ini dalam Bahasa Indonesia.`,
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      <header>
        ...
        <p className="text-slate-500">Wawasan pintar dari data bisnis akun Anda.</p>
      </header>
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm min-h-[400px] flex flex-col">
        {error && <div className="p-4 bg-red-50 text-red-600 rounded-xl mb-4 border border-red-100 flex items-center gap-2 text-sm">{error}</div>}
        ...
        {!insight && !loading && (
          <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <div className="max-w-md">
              ...
              <p className="text-slate-500 mt-2 text-sm">AI akan menganalisis data berdasarkan filter di atas.</p>
            </div>
            <button onClick={generateInsights} className="mt-4 bg-indigo-600 text-white px-8 py-3 rounded-xl font-semibold hover:bg-indigo-700 active:scale-95 shadow-lg shadow-indigo-100 transition-all">Mulai Analisis Sekarang</button>
          </div>
        )}

        {loading && <div className="flex-1 flex flex-col items-center justify-center text-center space-y-4"><Loader2 className="w-12 h-12 text-indigo-600 animate-spin" /><p className="text-slate-500 font-medium text-sm animate-pulse">Menghubungkan ke Gemini AI...</p></div>}
        
        {insight && !loading && (
          <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4">
              <h3 className="font-bold text-slate-900 text-xs uppercase tracking-widest text-indigo-600">Laporan Strategis</h3>
              <button onClick={generateInsights} className="text-indigo-600 text-xs font-bold flex items-center gap-1 hover:underline"><RefreshCw className="w-4 h-4" /> Perbarui</button>
            </div>
            <div className="prose prose-slate max-w-none whitespace-pre-wrap text-slate-700 leading-relaxed font-medium text-sm">{insight}</div>
          </div>
        )}
      </div>
    </div>
