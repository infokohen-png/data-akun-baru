  ...
  return (
    <div className={`min-h-screen flex flex-col relative overflow-hidden transition-colors duration-700 ${isDarkMode ? 'bg-[#0f111a]' : 'bg-slate-50'}`}>
      ...
      <div className="relative z-10 flex flex-col h-full min-h-screen max-w-2xl mx-auto w-full p-6 md:p-10">
        ...
        {/* Header Sejajar: Logo & Judul di Pojok Kiri Atas */}
        ...
        {/* Content Section: Mengembalikan ke 2 Kategori Utama dengan Layout Gambar */}
        <main className="flex-1 flex flex-col gap-6">
          <div className="mb-4">
             ...
             <p className={`text-xs mt-2 font-medium ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
              Silakan pilih modul aplikasi yang ingin Anda akses hari ini.
             </p>
          </div>

          <div className="space-y-4">
            {/* Card: DATA AKUN (BUSINESS) */}
            <div 
              onClick={() => onSelect('BUSINESS')}
              className={`group relative p-6 md:p-8 rounded-[2rem] border-2 cursor-pointer transition-all duration-300 flex items-start gap-5 ${
                isDarkMode 
            ...
            >
              <div className={`w-16 h-16 rounded-[1.25rem] flex-shrink-0 flex items-center justify-center transition-transform group-hover:rotate-3 ${isDarkMode ? 'bg-[#0f111a] text-amber-500 border border-slate-800' : 'bg-amber-50 text-amber-600'}`}>
                <Building2 className="w-8 h-8" />
              </div>

              <div className="flex-1 min-w-0 pt-1">
                <h3 className={`text-xl font-black leading-tight mb-2 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                  DATA AKUN
                </h3>
                <p className={`text-xs font-medium leading-relaxed mb-6 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                  Platform untuk mengelola administrasi toko, produk, penjualan harian, dan ringkasan omset bisnis Anda.
                </p>
                
                <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-amber-500 group-hover:text-amber-400 transition-colors">
                  Buka Aplikasi <ArrowRight className="w-4 h-4 group-hover:translate-x-1.5 transition-transform" />
                </div>
              </div>
              <Sparkles className={`absolute top-6 right-6 w-4 h-4 transition-opacity ${isDarkMode ? 'text-amber-500/10' : 'text-amber-500/20'}`} />
            </div>
            ...
            >
              <div className={`w-16 h-16 rounded-[1.25rem] flex-shrink-0 flex items-center justify-center transition-transform group-hover:rotate-3 ${isDarkMode ? 'bg-[#0f111a] text-indigo-500 border border-slate-800' : 'bg-indigo-50 text-indigo-600'}`}>
              ...
              <div className="flex-1 min-w-0 pt-1">
                ...
                <p className={`text-xs font-medium leading-relaxed mb-6 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>
                  Sistem monitoring KPI talent, target konten harian, dan manajemen performa tim kreatif secara terpadu.
                </p>
                
                <div className="flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-indigo-500 group-hover:text-indigo-400 transition-colors">
                  Buka Aplikasi <ArrowRight className="w-4 h-4 group-hover:translate-x-1.5 transition-transform" />
                </div>
              </div>
            </div>
