...
import { 
  ...
  CheckSquare, FileText, ArrowLeft, Users2, LayoutGrid, Home, Layers, Target
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const menuItems = appMode === 'BUSINESS' ? businessItems : talentItems;
  ...
  return (
    <>
      <div className={`md:hidden fixed top-0 left-0 right-0 z-40 border-b transition-colors h-14 px-4 flex items-center justify-between ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
        <span className={`font-black text-xl ${isDarkMode ? 'text-indigo-500' : 'text-slate-900'}`}>PT BMS</span>
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 z-[60] flex flex-col border-r transition-transform md:sticky md:top-0 md:h-screen md:translate-x-0 md:w-64 ${isDarkMode ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200'} ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 flex flex-col h-full">
          <div className="flex items-center justify-between mb-8">
            <h1 className={`text-2xl font-black ${isDarkMode ? 'text-indigo-500' : 'text-slate-900'}`}>PT BMS</h1>
          ...
          <div className="mb-6 space-y-4">
             <button 
               ...
               className={`w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl ${isDarkMode ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' : 'bg-indigo-600 text-white'} shadow-lg active:scale-95 transition-all`}
             >
               {appMode === 'BUSINESS' ? (
                 <><LayoutGrid className="w-4 h-4" /> GANTI PROYEK</>
               ) : (
