      ...
      >
        <div className="flex items-center justify-between h-14 px-4">
          <div className="flex items-center gap-2">
            <span className={`font-black tracking-tighter text-xl ${isDarkMode ? 'text-amber-500' : 'text-slate-900'}`}>PT BMS</span>
          </div>
      ...
      `}>
        <div className="p-6">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              <h1 className={`text-2xl font-black tracking-tighter ${isDarkMode ? 'text-amber-500' : 'text-slate-900'}`}>PT BMS</h1>
            </div>
            ...
            {menuItems.map((item) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-xl transition-all ${
                  currentView === item.id 
                  ? (isDarkMode ? 'bg-amber-500/10 text-amber-500 font-bold' : 'bg-amber-50 text-amber-700 shadow-sm font-bold') 
                  : (isDarkMode ? 'text-slate-400 hover:bg-slate-800' : 'text-slate-600 hover:bg-slate-50')
                }`}
              >
                <item.icon className={`w-5 h-5 ${currentView === item.id ? 'text-amber-600' : (isDarkMode ? 'text-slate-500' : 'text-slate-400')}`} />
                {item.label}
              </button>
            ))}
          </nav>
        ...
        <div className={`mt-auto p-4 border-t ${isDarkMode ? 'border-slate-800 bg-slate-900/20' : 'border-slate-100 bg-slate-50/50'}`}>
          <div className="flex items-center gap-3 mb-4 px-2">
            <div className="w-10 h-10 rounded-full bg-amber-500 flex items-center justify-center text-slate-900 font-bold border-2 border-white shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-amber-600 font-bold uppercase tracking-widest">Akun Aktif</p>
            </div>
          </div>
                ...
                <>
                  <div className="max-h-60 overflow-y-auto space-y-2 pr-1 no-scrollbar">
                    {profiles.map((profile) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-xs ${activeProfile?.id === profile.id ? 'bg-amber-500 text-slate-900' : 'bg-slate-800 text-slate-500'}`}>
                            {profile.nama[0].toUpperCase()}
                          </div>
                          <span className={`text-sm font-bold ${activeProfile?.id === profile.id ? 'text-amber-500' : (isDarkMode ? 'text-slate-300' : 'text-slate-700')}`}>
                            {profile.nama}
                          </span>
                        </div>
                        {activeProfile?.id === profile.id && <Check className="w-4 h-4 text-amber-500" />}
                      </button>
                    ))}
                  </div>
